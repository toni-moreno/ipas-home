...
var (
	...
	race                  bool   = false
	workingDir            string
	serverBinaryName      string = "ipashome"
)
...
func makeLatestDistCopies() {
	...
	runError("cp", "dist/ipashome_"+version+"_amd64.deb", "dist/ipashome_latest_amd64.deb")
	runError("cp", "dist/ipashome-"+linuxPackageVersion+rpmIteration+".x86_64.rpm", "dist/ipashome-latest-1.x86_64.rpm")
	runError("cp", "dist/ipashome-"+version+".linux-x64.tar.gz", "dist/ipashome-latest.linux-x64.tar.gz")
}
...
func createDebPackages() {
	createFpmPackage(linuxPackageOptions{
		packageType:            "deb",
		homeDir:                "/usr/share/ipashome",
		binPath:                "/usr/sbin/ipashome",
		configDir:              "/etc/ipashome",
		configFilePath:         "/etc/ipashome/ipashome.toml",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/ipashome",
		initdScriptFilePath:    "/etc/init.d/ipashome",
		systemdServiceFilePath: "/usr/lib/systemd/system/ipashome.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/ipashome",
		defaultFileSrc: "packaging/deb/default/ipashome",
		systemdFileSrc: "packaging/deb/systemd/ipashome.service",
...
func createRpmPackages() {
	createFpmPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/ipashome",
		initdScriptFilePath:    "/etc/init.d/ipashome",
		systemdServiceFilePath: "/usr/lib/systemd/system/ipashome.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/ipashome",
		defaultFileSrc: "packaging/rpm/sysconfig/ipashome",
		systemdFileSrc: "packaging/rpm/systemd/ipashome.service",
...
func createMinTar() {
	packageRoot, _ := ioutil.TempDir("", "ipashome-linux-pack")
	// create directories
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/ipashome"))
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/ipashome/conf"))
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/ipashome/bin"))
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/ipashome/log"))
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/opt/ipashome/public"))
	runPrint("cp", "conf/sample.ipashome.toml", filepath.Join(packageRoot, "/opt/ipashome/conf"))
	runPrint("cp", "bin/ipashome", filepath.Join(packageRoot, "/opt/ipashome/bin"))
	runPrint("cp", "bin/ipashome.md5", filepath.Join(packageRoot, "/opt/ipashome/bin"))
	runPrint("cp", "-a", filepath.Join(workingDir, "public")+"/.", filepath.Join(packageRoot, "/opt/ipashome/public"))
	tarname := fmt.Sprintf("dist/ipashome-%s-%s_%s_%s.tar.gz", version, getGitSha(), runtime.GOOS, runtime.GOARCH)
...
func createFpmPackage(options linuxPackageOptions) {
	...
	// copy sample ini file to /etc/ipashome
	runPrint("cp", "conf/sample.ipashome.toml", filepath.Join(packageRoot, options.configFilePath))
...
func clean() {
	...
	rmr("public")
	//rmr("tmp")
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/ipashome", goos, goarch)))
}
