...
func init() {
	...
	if _, err := os.Stat(configFile); err == nil {
		...
		viper.AddConfigPath("/etc/ipashome/")
		viper.AddConfigPath("/opt/ipashome/conf/")
	...
	if len(cfg.General.LogDir) > 0 {
		...
		//Log output
		f, _ := os.OpenFile(logDir+"/ipashome.log", os.O_APPEND|os.O_CREATE|os.O_RDWR, 0644)
		log.Out = f
	}
